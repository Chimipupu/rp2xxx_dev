...
void drv_ds3231_get_time_date(tm *p_tm)
{
    ...
    p_tm->tm_sec  = bcd_to_dec(Wire1.read() & 0x7F);
    p_tm->tm_min  = bcd_to_dec(Wire1.read() & 0x7F);
    p_tm->tm_hour = bcd_to_dec(Wire1.read() & 0x1F);
    p_tm->tm_wday = bcd_to_dec(Wire1.read() & 0x07);
    p_tm->tm_mday = bcd_to_dec(Wire1.read() & 0x3F);
    p_tm->tm_mon  = bcd_to_dec(Wire1.read() & 0x1F);
    p_tm->tm_year = bcd_to_dec(Wire1.read() & 0xFF);
}
...
void drv_ds3231_set_time_date(tm *p_tm)
{
    ...
    Wire1.write(dec_to_bcd(p_tm->tm_mon));
    Wire1.write(dec_to_bcd(p_tm->tm_year));
    Wire1.endTransmission();
}